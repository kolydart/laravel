...
namespace Kolydart\Laravel\App\Livewire\PowerGrid\V6;
...
/**
 * PowerGrid 6 compatible Audit Log component (Livewire 3).
 *
 * PowerGrid 6 moved several classes and changed the setUp() API
 * (PowerGrid facade instead of Header/Footer/Exportable::make()).
 * This variant lives on the PowerGrid-version axis, decoupled from
 * the Livewire-version (V2/V3) folders.
 *
 * Rendered via the Livewire alias "pg-audit-log", optionally with a
 * :model parameter and lazy loading.
 */
final class PgAuditLog extends PowerGridComponent
{
    ...
    // PowerGrid 6 requires a unique table name per component
    public string $tableName = 'pg_audit_log';
    ...
    */
    public function setUp(): array
    {
        ...
        if (auth()?->user()?->roles()?->pluck('id')->contains(1) ?? false) {
            $arr[] = PowerGrid::exportable('export')
                ->striped()
                ->type(Exportable::TYPE_XLS, Exportable::TYPE_CSV);
        }

        $arr[] = PowerGrid::header()
            ->showToggleColumns()
            ->showSearchInput();

        $arr[] = PowerGrid::footer()
            ->showPerPage()
            ->showRecordCount();
    ...
    */
    public function datasource(): Builder
    {
        ...
        $query = $AuditLog::query()
            ...
            ->select('audit_logs.*')
            // Deduplicate identical entries (one row per group) without
            // violating only_full_group_by: keep the latest id of each group.
            // PowerGrid 6 wraps the datasource in a count(*) aggregate, which
            // breaks a GROUP BY + "select *" under strict MySQL sql_mode.
    ...
    public function actions($row): array
    {
       return [
            Button::add('view')
                ...
                ->id('action_view_'.$row->id)
                ->class('btn btn-xs btn-primary')
                ->tag('a')
                ->route('admin.audit-logs.show', [$row->id], '_blank')
